...
<template>
  <div class="relative w-full" ref="containerRef">
    <label v-if="label" :for="id" class="block text-[10px] font-extrabold uppercase tracking-wider text-gray-500 mb-1">
      {{ label }}
    </label>
    ...
    >
      <div
        v-if="isOpen"
        class="absolute left-0 top-full mt-1.5 w-full bg-white rounded-2xl shadow-2xl border border-gray-200 z-[300] overflow-hidden text-gray-800"
      >
        <!-- Search Input Header -->
        <div class="p-2.5 border-b border-gray-100 bg-gray-50">
          <div class="relative">
            <input
              ...
              class="w-full pl-9 pr-3 py-2 text-xs text-gray-800 bg-white border border-gray-200 rounded-xl outline-none focus:border-[#074C72] font-medium"
              @click.stop
            />
        ...
        <div class="max-h-60 overflow-y-auto divide-y divide-gray-50 no-scrollbar">
          ...
          >
            ...
            <span class="text-[11px] text-gray-400 font-medium shrink-0 ml-2">
              {{ item.country }}
            </span>
          </button>

          <div v-if="filteredCities.length === 0" class="p-4 text-center text-xs text-gray-400">
