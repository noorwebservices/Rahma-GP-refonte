<template>
  <div class="space-y-6">
    ...
    <div class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-3xl p-5 shadow-2xs flex flex-col md:flex-row items-center justify-between gap-4">
      ...
      <select v-model="filters.statut" @change="fetchDemandes" class="bg-[#FAF7F2] dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-2xl px-4 py-2.5 text-xs font-bold text-[#053754] dark:text-slate-100 outline-none cursor-pointer">
    ...
    <div class="bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-3xl shadow-2xs overflow-hidden">
      ...
      <div v-if="loading" class="p-12 text-center space-y-3">
        ...
        <p class="text-xs font-bold text-[#074C72] dark:text-sky-300">Chargement des demandes de partenariat...</p>
      </div>

      <!-- Empty State -->
      <div v-else-if="demandes.length === 0" class="text-center py-16 text-gray-400 dark:text-slate-400 font-semibold text-sm">
      ...
      <div v-else class="overflow-x-auto">
        <table class="w-full min-w-[850px] text-left text-xs sm:text-sm">
          ...
          <tbody class="divide-y divide-gray-100 dark:divide-slate-800">
            <tr v-for="d in paginatedDemandes" :key="d.id" class="hover:bg-slate-50/80 dark:hover:bg-slate-800/50 transition-colors">
              
              <!-- Entreprise -->
              <td class="px-6 py-4 whitespace-nowrap">
                <p class="font-extrabold text-[#053754] dark:text-slate-100 text-xs sm:text-sm whitespace-nowrap">{{ d.entreprise || 'Entreprise / Transporteur' }}</p>
                <p class="text-xs text-gray-500 dark:text-slate-400 font-medium whitespace-nowrap">Responsable : {{ d.nom_complet }}</p>
              </td>

              <!-- Contact -->
              <td class="px-6 py-4 text-xs font-medium whitespace-nowrap">
                <p class="text-[#074C72] dark:text-sky-300 font-bold whitespace-nowrap">{{ d.email }}</p>
                <p class="text-gray-500 dark:text-slate-400 font-mono font-bold whitespace-nowrap">{{ d.telephone }}</p>
              </td>

              <!-- Type Partenariat -->
              <td class="px-6 py-4 whitespace-nowrap">
                <span class="px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider bg-sky-50 dark:bg-sky-950/80 text-[#074C72] dark:text-sky-300 border border-sky-200 dark:border-sky-800 whitespace-nowrap">
                  {{ getTypeLabel(d) }}
                </span>
              </td>
              ...
              <td class="px-6 py-4 text-right whitespace-nowrap">
                <button @click="openModal(d)" class="px-3.5 py-1.5 rounded-xl text-xs font-extrabold bg-[#053754] hover:bg-[#074C72] dark:bg-sky-600 dark:hover:bg-sky-500 text-white transition-all shadow-xs cursor-pointer flex items-center gap-1.5 ml-auto whitespace-nowrap">
    ...
    <Teleport to="body">
      <div v-if="selectedDemande" class="fixed inset-0 z-[999] flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm" @click.self="selectedDemande = null">
        <div class="w-full max-w-2xl bg-white dark:bg-slate-900 border border-gray-200 dark:border-slate-800 rounded-3xl shadow-2xl p-6 sm:p-8 space-y-6 text-gray-800 dark:text-slate-200 animate-in fade-in duration-200 max-h-[90vh] overflow-y-auto">
          
          <!-- Modal Header -->
          <div class="flex items-start justify-between border-b border-gray-100 dark:border-slate-800 pb-5">
            <div class="space-y-1">
              <span class="px-3 py-1 rounded-full text-xs font-black uppercase tracking-wider bg-[#D8ECF8] dark:bg-sky-950 text-[#074C72] dark:text-sky-300 border border-[#074C72]/20 dark:border-sky-800">
                {{ getTypeLabel(selectedDemande) }}
              </span>
              <h3 class="text-xl sm:text-2xl font-black text-[#053754] dark:text-sky-300 mt-2">
                {{ selectedDemande.entreprise || selectedDemande.nom_complet }}
              </h3>
              <p class="text-xs text-gray-500 dark:text-slate-400 font-medium">Demande de partenariat professionnel soumise le {{ formatDate(selectedDemande.created_at) }}</p>
            </div>
            <button @click="selectedDemande = null" class="w-9 h-9 rounded-full bg-gray-100 dark:bg-slate-800 hover:bg-gray-200 dark:hover:bg-slate-700 text-gray-600 dark:text-slate-300 font-bold text-base flex items-center justify-center transition">✕</button>
          </div>
          ...
          <div class="space-y-5 text-xs">
            ...
            <div class="bg-[#FAF7F2] dark:bg-slate-800/80 p-5 rounded-2xl border border-gray-200 dark:border-slate-700 space-y-3">
              <h4 class="font-extrabold text-xs uppercase tracking-wider text-[#053754] dark:text-sky-300 flex items-center gap-2">
                <span>👤 Responsable & Contact Direct</span>
              </h4>

              <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div class="space-y-0.5">
                  <span class="text-gray-400 dark:text-slate-400 font-bold uppercase text-[10px]">Nom Complet</span>
                  <p class="text-[#053754] dark:text-slate-100 font-extrabold text-sm">{{ selectedDemande.nom_complet }}</p>
                </div>

                <div class="space-y-0.5">
                  <span class="text-gray-400 dark:text-slate-400 font-bold uppercase text-[10px]">Statut Actuel</span>
                  <div>
                ...
                <div class="space-y-0.5">
                  <span class="text-gray-400 dark:text-slate-400 font-bold uppercase text-[10px]">Email Professionnel</span>
                  <a :href="`mailto:${selectedDemande.email}`" class="text-[#074C72] dark:text-sky-300 hover:underline font-bold text-xs flex items-center gap-1">
                    <span>✉️ {{ selectedDemande.email }}</span>
                  </a>
                </div>

                <div class="space-y-0.5">
                  <span class="text-gray-400 dark:text-slate-400 font-bold uppercase text-[10px]">Numéro Téléphone</span>
                  <a :href="`tel:${selectedDemande.telephone}`" class="text-[#074C72] dark:text-sky-300 hover:underline font-bold text-xs font-mono flex items-center gap-1">
            ...
            <div class="space-y-4">
              ...
              <div v-if="parsedDetails.formula" class="p-4 bg-amber-50 dark:bg-amber-950/80 border border-amber-200 dark:border-amber-800 rounded-2xl space-y-1">
                <span class="text-[10px] font-extrabold uppercase tracking-wider text-amber-800 dark:text-amber-300 block">Offre / Formule Souhaitée</span>
                <p class="text-amber-950 dark:text-amber-100 font-black text-sm">{{ parsedDetails.formula }}</p>
              </div>
              ...
              <div v-if="parsedDetails.address || parsedDetails.frequency" class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div v-if="parsedDetails.address" class="p-4 bg-[#FAF7F2] dark:bg-slate-800/80 border border-gray-200 dark:border-slate-700 rounded-2xl space-y-1">
                  <span class="text-[10px] font-extrabold uppercase tracking-wider text-gray-400 dark:text-slate-400 block">Adresse / Zone d'Activité</span>
                  <p class="text-[#053754] dark:text-slate-100 font-extrabold text-xs sm:text-sm">📍 {{ parsedDetails.address }}</p>
                </div>

                <div v-if="parsedDetails.frequency" class="p-4 bg-[#FAF7F2] dark:bg-slate-800/80 border border-gray-200 dark:border-slate-700 rounded-2xl space-y-1">
                  <span class="text-[10px] font-extrabold uppercase tracking-wider text-gray-400 dark:text-slate-400 block">Fréquence de Transport</span>
                  <p class="text-[#074C72] dark:text-sky-300 font-extrabold text-xs sm:text-sm">✈️ {{ parsedDetails.frequency }}</p>
                </div>
              </div>

              <!-- Message Texte -->
              <div class="bg-white dark:bg-slate-800 p-5 rounded-2xl border-l-4 border-l-[#053754] dark:border-l-sky-400 border border-gray-200 dark:border-slate-700 shadow-2xs space-y-2">
                <span class="text-[#053754] dark:text-sky-300 font-extrabold uppercase tracking-wider block text-[11px]">
                  💬 Message & Motivation de la Demande
                </span>
                <div class="p-4 bg-[#FAF7F2] dark:bg-slate-900 rounded-xl text-gray-700 dark:text-slate-200 leading-relaxed font-medium text-sm whitespace-pre-line border border-gray-100 dark:border-slate-800">
          ...
          <!-- Actions Modération -->
          <div class="flex flex-wrap items-center justify-between gap-3 pt-4 border-t border-gray-100 dark:border-slate-800">
            <span class="text-xs text-gray-500 dark:text-slate-400 font-semibold">Changer l'état du traitement :</span>
            ...
            <div class="flex items-center gap-2">
              ...
              <div v-if="selectedDemande.statut === 'traite'" class="flex items-center gap-2 px-4 py-2 rounded-xl bg-emerald-50 dark:bg-emerald-950/80 border border-emerald-200 dark:border-emerald-800 text-emerald-700 dark:text-emerald-300 font-extrabold text-xs">
