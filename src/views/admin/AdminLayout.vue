<template>
  <div class="min-h-screen bg-[#FAF7F2] font-sans flex text-gray-800 selection:bg-[#B50302] selection:text-white">
    ...
    <aside 
      class="fixed lg:sticky top-0 left-0 z-50 h-screen w-64 bg-white border-r border-gray-200 flex flex-col justify-between transition-transform duration-300 ease-in-out shrink-0 shadow-sm"
      :class="isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'"
    >
      <div>
        <!-- Sidebar Logo Area (No "Espace Admin" text) -->
        <div class="p-6 border-b border-gray-100 flex items-center justify-between">
          ...
          <button @click="isMobileMenuOpen = false" class="lg:hidden text-gray-400 hover:text-gray-700 p-1">
            ✕
          </button>
        </div>
        ...
        <nav class="p-4 space-y-1.5">
          <router-link 
            ...
            :class="$route.path === '/admin' ? 'bg-[#053754] text-white shadow-md' : 'text-gray-600 hover:bg-slate-100 hover:text-[#053754]'"
          >
          ...
          <router-link 
            ...
            @click="isMobileMenuOpen = false"
            class="flex items-center px-4 py-3 rounded-2xl text-xs font-extrabold transition-all duration-200 cursor-pointer"
            :class="$route.path.startsWith('/admin/users') ? 'bg-[#053754] text-white shadow-md' : 'text-gray-600 hover:bg-slate-100 hover:text-[#053754]'"
          >
          ...
          <router-link 
            ...
            :class="$route.path.startsWith('/admin/signalements') ? 'bg-[#053754] text-white shadow-md' : 'text-gray-600 hover:bg-slate-100 hover:text-[#053754]'"
          >
          ...
          <router-link 
            ...
            @click="isMobileMenuOpen = false"
            class="flex items-center justify-between px-4 py-3 rounded-2xl text-xs font-extrabold transition-all duration-200 cursor-pointer"
            :class="$route.path.startsWith('/admin/partenariats') ? 'bg-[#053754] text-white shadow-md' : 'text-gray-600 hover:bg-slate-100 hover:text-[#053754]'"
          >
          ...
          <router-link 
            ...
            @click="isMobileMenuOpen = false"
            class="flex items-center px-4 py-3 rounded-2xl text-xs font-extrabold transition-all duration-200 cursor-pointer"
            :class="$route.path.startsWith('/admin/voyages') ? 'bg-[#053754] text-white shadow-md' : 'text-gray-600 hover:bg-slate-100 hover:text-[#053754]'"
          >
          ...
          <router-link 
            ...
            @click="isMobileMenuOpen = false"
            class="flex items-center px-4 py-3 rounded-2xl text-xs font-extrabold transition-all duration-200 cursor-pointer"
            :class="$route.path.startsWith('/admin/profile') ? 'bg-[#053754] text-white shadow-md' : 'text-gray-600 hover:bg-slate-100 hover:text-[#053754]'"
          >
      ...
      <div class="p-4 border-t border-gray-100 space-y-3">
        ...
        <router-link to="/client" class="w-full py-2.5 px-3 rounded-xl text-xs font-extrabold text-[#074C72] bg-[#D8ECF8] hover:bg-sky-200 transition-colors flex items-center justify-center gap-2 cursor-pointer">
          <span>➔</span>
        ...
        <div class="flex items-center justify-between p-3 rounded-2xl bg-slate-50 border border-slate-200">
          <div class="flex items-center gap-2.5 overflow-hidden">
            ...
            <div class="overflow-hidden">
              <p class="text-xs font-bold text-[#053754] truncate">{{ currentUser?.prenom }} {{ currentUser?.nom }}</p>
    ...
    <div class="flex-1 min-w-0 flex flex-col min-h-screen">
      ...
      <!-- Top Mobile Navbar -->
      <header class="h-16 bg-white border-b border-gray-200 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30 shadow-2xs">
        <div class="flex items-center gap-3">
          <button @click="isMobileMenuOpen = !isMobileMenuOpen" class="lg:hidden p-2 rounded-xl bg-gray-100 text-gray-700 hover:bg-gray-200">
            <svg class="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
            </svg>
          </button>
          
          <h1 class="font-extrabold text-base sm:text-lg text-[#053754]">
            {{ headerTitle }}
          </h1>
        </div>
        ...
        <div class="flex items-center gap-2">
          ...
          <div class="hidden sm:flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-[#F3F4F6] border border-gray-200 text-xs font-extrabold text-gray-700">
