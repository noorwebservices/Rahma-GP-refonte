...
<template>
  <div class="space-y-5 pb-16">
    ...
    <div class="space-y-1">
      <h1 class="text-xl sm:text-2xl font-serif font-bold text-principal-dark">Messagerie Client</h1>
      <p class="text-xs sm:text-sm text-gray-500">Échangez directement avec vos transporteurs GP pour vos réservations</p>
    </div>
    ...
    <div v-else-if="filteredConversations.length > 0" class="space-y-3">
      ...
      >
        ...
        <!-- RGP Avatar Circle -->
        <div class="w-12 h-12 sm:w-13 sm:h-13 rounded-full bg-[#053754] text-white flex items-center justify-center font-extrabold text-sm shrink-0 shadow-xs group-hover:scale-105 transition-transform">
          {{ conv.avatar }}
        </div>
    ...
    <div v-else class="bg-white rounded-3xl p-10 text-center border border-gray-200 space-y-2">
      ...
      <p class="text-xs text-gray-400">Vos échanges relatifs à vos réservations s'afficheront ici.</p>
    </div>
  </div>
</template>
