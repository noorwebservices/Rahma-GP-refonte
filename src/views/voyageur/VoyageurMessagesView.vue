...
const conversations = ref([
  {
    ...
    flagTo: '🇫🇷',
    parcelCode: '#RS-7729',
    lastMessage: 'Avec plaisir. Je vous confirme la réservation dès que la vérification est terminée.'
  },
  {
    id: 2,
    ...
    flagTo: '🇫🇷',
    parcelCode: '#RS-8830',
    lastMessage: 'Bonjour, le dépôt du pli de documents se fera bien aux Parcelles Assainies ?'
  }
])
  ...
  <div class="space-y-5 pb-16">
    ...
    <div class="relative">
      ...
      <input
        v-model="searchQuery"
        type="text"
        placeholder="Rechercher un client ou un numéro de colis"
    ...
    <div class="space-y-3">
      <div
        ...
        class="bg-white rounded-2xl p-4 border border-gray-200 hover:border-[#074C72]/40 transition-all shadow-2xs cursor-pointer flex items-center gap-3.5 group"
      >
        <!-- Avatar Circle -->
        <div class="w-13 h-13 sm:w-14 sm:h-14 rounded-full bg-[#B50302] text-white flex items-center justify-center font-extrabold text-sm sm:text-base shrink-0 shadow-xs group-hover:scale-105 transition-transform">
          {{ conv.avatar }}
        </div>
        ...
        <div class="flex-1 min-w-0 space-y-1.5">
          ...
          <div class="flex items-center justify-between">
            <h3 class="text-sm font-extrabold text-[#053754] truncate">{{ conv.clientName }}</h3>
            <span class="text-xs text-gray-400 font-medium shrink-0 ml-2">{{ conv.time }}</span>
          </div>

          <!-- Middle Row: Route + Parcel ID -->
          <div class="flex items-center justify-between text-xs">
            <div class="flex items-center gap-1.5 font-bold text-gray-800">
              <span>{{ conv.flagFrom }}</span>
              ...
              <span>{{ conv.flagTo }}</span>
              <span>{{ conv.routeTo }}</span>
            </div>
            <span class="font-extrabold text-[#053754] text-xs shrink-0">{{ conv.parcelCode }}</span>
          </div>
